...
进入或退出 Play Mode 时，Unity 会执行 Domain Reload 导致 WebSocket 临时断开。MCP Unity 现在透明地处理此过程 — `set_editor_state("play"/"stop")` 会自动等待重连并在单次调用中返回验证后的结果，无需手动重试。

### 外部工具插件系统

外部项目可以**无需修改 mcp-unity 包**即可注册自定义 MCP 工具。只需在您自己的程序集中创建继承 `McpToolBase` 的 C# 类，启动时会通过程序集扫描自动发现并在 Node.js 端动态注册。
...
- `update_scriptable_object`: 更新项目中已有 ScriptableObject 资源的字段值
  > **示例提示:** "将 Assets/Settings/GameSettings.asset 的 maxHealth 更新为 200"
...
- `set_sibling_index`: 设置 GameObject 在父对象中的兄弟索引（渲染顺序），用于控制 UI 元素层级
  > **示例提示:** "将 StartButton 移动到兄弟节点的第一个位置"

- `read_serialized_fields`: 使用 Unity 的 SerializedProperty API 读取组件的序列化字段。同时支持序列化名称（`m_Color`）和属性名称（`color`）
  > **示例提示:** "读取 MyButton 上 Image 组件的 color 和 raycastTarget 字段"

- `write_serialized_fields`: 使用 Unity 的 SerializedProperty API 写入组件的序列化字段。最适合 Unity 内置组件字段（`m_Color`、`m_Sprite` 等）
  > **示例提示:** "将 MyButton 的 Image 颜色设置为红色并禁用 raycast target"

- `batch_execute`: 在单个批处理请求中执行多个工具操作，减少往返次数并支持原子操作，失败时可选回滚
  > **示例提示:** "在单个批处理操作中创建 10 个名为 Enemy_1 到 Enemy_10 的空 GameObject"

- `get_interactable_elements`: 扫描场景中所有可交互的 UI 元素（Button、Toggle、InputField、Slider、Dropdown、ScrollRect 等），支持过滤和范围控制。需要 Play Mode
  > **示例提示:** "显示当前场景中所有可交互的 UI 元素"

- `simulate_pointer_click`: 对 UI 元素模拟完整的指针点击事件序列（PointerEnter → PointerDown → PointerUp → PointerClick → PointerExit）。需要 Play Mode
  > **示例提示:** "点击 Canvas 中的 LoginButton"

- `simulate_input_field`: 向 InputField 或 TMP_InputField 填入文本，触发 onValueChanged 以及可选的 onEndEdit/onSubmit 事件。需要 Play Mode
  > **示例提示:** "在 EmailField 中输入 'player@test.com'"

- `get_ui_element_state`: 查询单个 UI 元素的运行时状态，包括组件状态、RectTransform 信息和显示文本。在编辑模式和运行模式下均可使用
  > **示例提示:** "SubmitButton 的当前状态是什么？"

- `wait_for_condition`: 等待 GameObject 满足指定条件（active、inactive、exists、not_exists、interactable、text_equals、text_contains、component_enabled），支持可配置超时。需要 Play Mode
  > **示例提示:** "等待 LoadingPanel 变为不活跃状态"

- `simulate_drag`: 模拟 UI 元素的拖拽手势，支持多帧插值、像素偏移或基于目标的移动，以及 IDropHandler 支持。需要 Play Mode
  > **示例提示:** "将 InventoryItem 拖到 EquipSlot"
