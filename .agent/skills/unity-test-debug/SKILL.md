...
run_tests (預設 EditMode；僅例外用 PlayMode)
    ...
    ├─ EditMode ──→ 正常取得結果
    │
    └─ PlayMode ──→ 預期 MCP 斷線 → 等待重連
                     → get_console_logs 確認完成
                     → Read TestResults.xml 驗證結果
    │
    ▼
...
## PlayMode MCP 斷線問題（已知限制）

透過 MCP `run_tests` 執行 PlayMode 測試時，Unity 進入 Play Mode 會導致 MCP WebSocket 斷線，`run_tests` 回傳 `Connection failed: Unknown error` 或 `Request timed out`。**這是預期行為，不是錯誤。**

### 正確處理流程

1. 呼叫 `run_tests(testMode: "PlayMode", ...)`，預期收到連線錯誤。
2. **不要重試** `run_tests` — 測試已在 Unity 內執行中。
3. 等待 MCP 自動重連（Unity 退出 Play Mode 後恢復，通常 10~30 秒）。
4. 重連後用 `get_console_logs(logType: "info")` 確認測試完成（搜尋 `Saving results to:` 訊息）。
5. 讀取測試結果 XML（路徑出現在 console log 中），用 Read 工具解析。

### 常見錯誤做法（禁止）

- ❌ 收到斷線錯誤後立即重試 `run_tests`（會導致重複執行或再次斷線）
- ❌ 改用 CLI batch mode（`-runTests -batchmode`）— Unity Editor 已開啟時會因 project lock 失敗
- ❌ 因斷線就放棄 PlayMode 改用 EditMode — 如果測試需要 PlayMode，就用 PlayMode
