...
Test names may contain characters XML cannot hold (control characters, lone surrogates); NUnit writes them as a literal backslash-u sequence and the consistency gate compares callback names after applying the same escape. A preview-scene `Camera` added before being moved into the preview scene is enabled but dormant (absent from `Camera.allCameras`); `screenshot_game_view` leaves dormant cameras untouched. Tests that need a rendering orphan must register it with a real `enabled` false→true transition (see `WakeRenderingOrphan` in `ScreenshotToolsObservabilityTests`).

Unsaved scenes block Unity in two ways: UTF shows a save dialog at the start of every run, and saving an untitled scene opens a Save Scene file dialog. `run_tests` therefore refuses with `dirty_scenes_present` and non-additive `load_scene` refuses with `untitled_dirty_scene`; neither saves nor discards. Tests that exercise non-additive `load_scene` must use the `LoadSceneTool` seams (`DescribeDirtyScenes`, `SaveOpenScenes`, `OpenScene`) instead of replacing the runner scene, and `TestRunnerService` fake-API tests only check dirty scenes when they pass `dirtyScenes`.
